...
void parse_paramfile(char* fname)
{
    ...
    if (num_parameter_sets > 0) {
        opts = (int**)hcmalloc(sizeof(int*)* num_parameter_sets);
    } else {
        opts = (int**)hcmalloc(sizeof(int*)* N);
    }
    int m = (num_parameter_sets>0)?
            num_parameter_sets:N;
    for(i=0;i<m;i++) {
        opts[i] = (int*)hcmalloc(sizeof(int)*NUMOPTS);
...
void entry_fn(void* not_used)
{
    ...
    } else
    {
        ...
        int total = 0;
        int* remain = (int*) hcmalloc(sizeof(int)*num_parameter_sets);
        int i;
